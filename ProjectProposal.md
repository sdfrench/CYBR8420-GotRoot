...
### Motivation
---
Today’s consumers not only expect their password manager to be secure, be easy to use, work online, while supporting multiple types of devices. To work online, modern password managers are typically comprised of two major components, the client, which typically supports multiple hardware devices, and the server, which is 'online' (typically in the cloud) and is where the encrypted password vault is stored. This of course presents many more security challenges over the traditional password managers which stored the password vault locally.  In addition, password managers must offer features that make not make them easy to use but protect against unsafe practices. If they are too cumbersome or burdensome to use, a user will more that likely take shortcuts or not use at all. Most password managers offer unique and strong password generation, which helps protect against weak and sharing of password, which can allow attackers to easily compromise accounts. 
                   
When you consider how many login credentials a typical user has to keep track of these days, it is easy to see why a good password manager is so important to have. This software is one of the more popular online password managers on the market today, so it can inferred it has a good balance between ease of use, features, and security. However, unlike most of the others, it is completely open source. This transpaency is welcomed by savvy consumers who want to know exactly how their sensitive data is stored and protected both on the client and in the cloud. In addition if a consumer is not comfortable storing their sensitive data on the cloud, this software allow them to easily stand up a server stack on their own online host.

We chose this software due to its features, popularity and the fact it is open source. In fact one of our team members is actively using the tool. Even though this application has a reputation for good secuirity, appears to be well written, and has been subject to third party review, we welcome the challenge of looking at the security of this application using the perspectives taught in this course. 
