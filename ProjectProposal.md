...
When you consider how many login credentials a typical user has to keep track of these days, it is easy to see what a good password manager is so important to have. This software is not only one of the premier online password managers on the market today, so it must strike a good balance between ease of use, features and security.  However, unlike most others, it is completely open source. This transpaency is welcome by savvy consumers who want to know exactly how their sensitive data is stored and protected on the cloud. In addition if a consumer is not comfortable storing their sensitive data on the cloud, this software allow them to easily stand up the server  stack on their own online host.                    

We chose this software due to its popularity and the fact it is open source. In fact one of our team members is actively using the tool. Even though this application appears to be well written and has been subject to third party review, we welcome the challenge of looking at this application using the perspectives taught in this course to see if we feel it is indeed as secure as perceived. 
