...
Bitwarden, an open-source online password management service, has developed a good reputation for being secure over the years. We knew when we chose this application for our project, it was going to be a big undertaking and there would be no "low hangin fruit" given the maturity of the application coupled with the fact it is completely open source and has already been [audited](https://cdn.bitwarden.net/misc/Bitwarden%20Security%20Assessment%20Report%20-%20v2.pdf) by third party security company. 
...
Client/Server data was intercepted using a transparent TLS proxy within a virtual environment, using VMWare and two Linux Mint VMs. The first VM hosted the client application, while the second posed as a gateway. The gateway VM used a combination of iptable, NAT rules, and SSLsplit to facilitate the transparent proxy. OpenSSL binary was used to create self-signed certificate/key pairs, which in turn were used by SSLsplit to forge certificates for TLS sites accessed by the client. The Bitwarden CLI has a native CA trust store embedded and no functionality for modification We replaced one of the CA certificates with our self-signed certificate, using Ghex, dd, and cat. The modified binary was appropriately renamed to avoid confusion and padding was added to maintain the length of the certificate to avoid alignment issues. The commands below show the error with the native CA store versus the modified one. As it turns out, very few commands required the modified version, as the client uses a local copy of the vault which is synced after a successful login. 
